...
const SITE = {
  whatsapp: '966561940018',            // رقم واتساب المراد
  whatsappDisplay: '+966 56 194 0018', // صيغة العرض
...
// المحجوز لا يُحتسب (يُعرض متاحاً)؛ فقط المباع من جدول التحديثات الرسمي
const _s108 = { A1: 'مباع', A2: 'مباع', A3: 'مباع', A4: 'مباع', B1: 'متاح', B2: 'مباع', B3: 'متاح', B4: 'متاح', C1: 'متاح', C2: 'متاح', C3: 'متاح', C4: 'متاح', M1: 'متاح' };
...
// المحجوز لا يُحتسب (يُعرض متاحاً)؛ فقط المباع من جدول التحديثات الرسمي
const _s109 = { A1: 'متاح', A2: 'متاح', A3: 'متاح', A4: 'متاح', B1: 'متاح', B2: 'مباع', B3: 'مباع', B4: 'متاح', C1: 'متاح', C2: 'مباع', C3: 'مباع', C4: 'متاح', D1: 'متاح', D2: 'مباع', D3: 'مباع', D4: 'متاح', M1: 'متاح', M2: 'متاح' };
...
const _st106 = ['مباع', 'مباع', 'متاح', 'مباع', 'متاح', 'متاح', 'مباع', 'متاح', 'مباع', 'مباع'];
...
const PROJECTS = [
  ...
  {
    id: 108, code: '108', name: 'المراد هومز', district: 'النعيم', imgPos: 'center 72%',
    // رابط الموقع الدقيق على خرائط قوقل (يفتح الدبوس على المبنى مباشرة)
    mapLink: 'https://maps.app.goo.gl/5qzGn4kQyWF9661R8',
...
/* ── الوحدات المتاحة فعلياً (تُشتق من المشاريع الحقيقية) ─────────
   نجمع وحدات كل مشروع الحاملة للحالة «متاح» في عروض حسب النوع والمساحة
   والسعر، ويشير كل عرض إلى صفحة مشروعه. */
function availableOffers() {
  const offers = [];
  PROJECTS.forEach((p) => {
    if (!p.units || !p.units.length || !p.images || !p.images.length) return;
    const imgs = p.images.filter((s) => !/plan|investment/.test(s));
    const order = [];
    const groups = {};
    p.units.forEach((u) => {
      if (u.status !== 'متاح') return;
      const key = u.type + '|' + u.area + '|' + u.price;
      if (!groups[key]) {
        groups[key] = {
          projectId: p.id, projectName: p.name, district: p.district,
          type: u.type, typeGroup: u.type.indexOf('ملحق') === 0 ? 'ملحق' : u.type,
          area: u.area, rooms: u.rooms, baths: u.baths, price: u.price, count: 0,
        };
        order.push(key);
      }
      groups[key].count++;
    ...
    order.forEach((k, i) => {
      const g = groups[k];
      g.image = imgs.length ? imgs[i % imgs.length] : p.images[0];
      offers.push(g);
    });
  });
  return offers;
...
function availableUnitsTotal() {
  return PROJECTS.reduce((n, p) =>
    n + ((p.units || []).filter((u) => u.status === 'متاح').length), 0);
}
